...
const mockSubjects: Subject[] = [
    ...
    { id: 'sub3', name: 'Mathematics', classId: 'cls8' }, // Corrected classId for consistency
...
export function Subjects() {
  ...
  const handleAddSubject = async () => {
    ...
    if (subjects.some(s => s.name.trim().toLowerCase() === newSubjectName.trim().toLowerCase() && s.classId === selectedClassId)) {
       toast({ title: "Error", description: `Subject "${newSubjectName.trim()}" already exists for this class.`, variant: "destructive" });
        return;
    }
    ...
    <Card>
      ...
      <CardContent>
        ...
        <div className="flex items-center gap-2 mb-4">
           <Label htmlFor="filterClassSelect">Filter by Class:</Label> {/* Added htmlFor */}
          ...
          <Table>
            ...
            <TableBody>
              ...
              {filteredSubjects.sort((a,b) => a.name.localeCompare(b.name)).map((subject) => (
                <TableRow key={subject.id} className="hover:bg-muted/50 transition-colors"> {/* Added hover effect */}
                  ...
                  <TableCell>
                    {subject.syllabusFileUrl ? (
                      <div className="flex items-center gap-2">
                        ...
                        <Button variant="secondary" size="sm" onClick={() => triggerFileUpload(subject.id)} className="hover:bg-secondary/80">
